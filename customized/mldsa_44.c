...
void sha256(const uint8_t *text, uint8_t *md) {
  ...
  md_type = EVP_sha256();  // SHA-256 is available in OpenSSL
...
void convert_size_t_to_uint8_array(size_t x, uint8_t array[8]) {
  ...
  // Chuyển đổi size_t x thành mảng uint8_t array[8] (lưu từ byte thấp đến byte cao)
  for (int i = 0; i < 8; i++) {
...
jbyteArray genKey(JNIEnv *env) {
  ...
  if (rc != OQS_SUCCESS) {
      fprintf(stderr, "ERROR: OQS_SIG_ml_dsa_44_keypair failed!\n");
      cleanup_stack(secret_key, OQS_SIG_ml_dsa_44_length_secret_key);
  }

  jbyteArray jResult = (*env)->NewByteArray(env, OQS_SIG_ml_dsa_44_length_public_key + OQS_SIG_ml_dsa_44_length_secret_key);
  if (jResult == NULL) {
      return NULL;
  }

  (*env)->SetByteArrayRegion(env, jResult, 0, OQS_SIG_ml_dsa_44_length_public_key, (const jbyte *)public_key);
  (*env)->SetByteArrayRegion(env, jResult, OQS_SIG_ml_dsa_44_length_public_key, OQS_SIG_ml_dsa_44_length_secret_key, (const jbyte *)secret_key);
...
jbyteArray sign(JNIEnv *env, jbyte *input) {
  // Extract secret key
  for (size_t i = PREFIX_LEN; i < PREFIX_LEN + OQS_SIG_ml_dsa_44_length_secret_key; i++) {
    secret_key[i - PREFIX_LEN] = input[i];
  }

  // Extract message
  for (size_t i = PREFIX_LEN + OQS_SIG_ml_dsa_44_length_secret_key; i < PREFIX_LEN + OQS_SIG_ml_dsa_44_length_secret_key + MESSAGE_LEN; i++) {
    message[i - (PREFIX_LEN + OQS_SIG_ml_dsa_44_length_secret_key)] = input[i];
  }

  rc = OQS_SIG_ml_dsa_44_sign(signature, &signature_len, message, message_len, secret_key);

  if (rc != OQS_SUCCESS) {
      fprintf(stderr, "ERROR: OQS_SIG_ml_dsa_44_sign failed!\n");
      cleanup_stack(secret_key, OQS_SIG_ml_dsa_44_length_secret_key);
  }

  jbyteArray jResult = (*env)->NewByteArray(env, OQS_SIG_ml_dsa_44_length_signature + 8); // 8 bytes for sig length
  if (jResult == NULL) {
      return NULL;
  }
  ...
  (*env)->SetByteArrayRegion(env, jResult, 0, OQS_SIG_ml_dsa_44_length_signature, (const jbyte *)signature);
  (*env)->SetByteArrayRegion(env, jResult, OQS_SIG_ml_dsa_44_length_signature, 8, (const jbyte *)st2bytes);
...
jbyteArray verify(JNIEnv *env, jbyte *input) {
  // Extract public key
  for (size_t i = PREFIX_LEN; i < PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key; i++) {
    public_key[i - PREFIX_LEN] = input[i];
  }

  // Extract message
  for (size_t i = PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key; i < PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key + MESSAGE_LEN; i++) {
    message[i - (PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key)] = input[i];
  }

  // Extract signature
  for (size_t i = PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key + MESSAGE_LEN; i < PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key + MESSAGE_LEN + OQS_SIG_ml_dsa_44_length_signature; i++) {
    signature[i - (PREFIX_LEN + OQS_SIG_ml_dsa_44_length_public_key + MESSAGE_LEN)] = input[i];
  }
  ...
  jbyteArray jResult = (*env)->NewByteArray(env, 1); // 1 byte for boolean status of verification
  if (jResult == NULL) {
      return NULL;
  }
  ...
  if (rc != OQS_SUCCESS) {
    fprintf(stderr, "ERROR: OQS_SIG_ml_dsa_44_verify failed!\n");
    cleanup_stack(secret_key, OQS_SIG_ml_dsa_44_length_secret_key);
    verifyStatus[0] = 0;
  }
  ...
  /*
    Data format:
      genKey():
        ...
        response: Pkey || Skey
      sign():
        request: Skey || Message<Must padding with \x00>
        response: Signature || Sig length
      verify():
        request: Pkey || Message<Must padding with \x00> || Signature
        response: Verify status <true or false>

    Note that '||' is the concatenation symbol.
  */
