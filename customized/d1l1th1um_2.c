...
jbyteArray genKey(JNIEnv *env) {
  ...
  jbyteArray jResult = (*env)->NewByteArray(env, OQS_SIG_dilithium_2_length_public_key + OQS_SIG_dilithium_2_length_secret_key);
  if (jResult == NULL) {
      return NULL;
  }

  (*env)->SetByteArrayRegion(env, jResult, 0, OQS_SIG_dilithium_2_length_public_key, (const jbyte *)public_key);
  (*env)->SetByteArrayRegion(env, jResult, OQS_SIG_dilithium_2_length_public_key, OQS_SIG_dilithium_2_length_secret_key, (const jbyte *)secret_key);
...
jbyteArray sign(JNIEnv *env, jbyte *input) {
  // Extract secret key
  for (size_t i = PREFIX_LEN; i < PREFIX_LEN + OQS_SIG_dilithium_2_length_secret_key; i++) {
    secret_key[i - PREFIX_LEN] = input[i];
  }

  // Extract message
  for (size_t i = PREFIX_LEN + OQS_SIG_dilithium_2_length_secret_key; i < PREFIX_LEN + OQS_SIG_dilithium_2_length_secret_key + MESSAGE_LEN; i++) {
    message[i - (PREFIX_LEN + OQS_SIG_dilithium_2_length_secret_key)] = input[i];
  }
  ...
  jbyteArray jResult = (*env)->NewByteArray(env, OQS_SIG_dilithium_2_length_signature + 8); // 8 bytes for sig length
  if (jResult == NULL) {
      return NULL;
  }
  ...
  (*env)->SetByteArrayRegion(env, jResult, 0, OQS_SIG_dilithium_2_length_signature, (const jbyte *)signature);
  (*env)->SetByteArrayRegion(env, jResult, OQS_SIG_dilithium_2_length_signature, 8, (const jbyte *)st2bytes);
...
jbyteArray verify(JNIEnv *env, jbyte *input) {
  // Extract public key
  for (size_t i = PREFIX_LEN; i < PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key; i++) {
    public_key[i - PREFIX_LEN] = input[i];
  }

  // Extract message
  for (size_t i = PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key; i < PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key + MESSAGE_LEN; i++) {
    message[i - (PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key)] = input[i];
  }

  // Extract signature
  for (size_t i = PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key + MESSAGE_LEN; i < PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key + MESSAGE_LEN + OQS_SIG_dilithium_2_length_signature; i++) {
    signature[i - (PREFIX_LEN + OQS_SIG_dilithium_2_length_public_key + MESSAGE_LEN)] = input[i];
  }
